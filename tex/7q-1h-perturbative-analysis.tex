...
\thmproof{
The leading terms omitted from $\Y$ and from $E_\mr{c}=\ip{\Y|H_\mr{c}|\Y}$ when we truncate at $m$-tuples are
\begin{align}
  \Y
=
  (
...
+
  \cd
\end{align}
yielding errors of $\ceil{(m+1)/2}$ in the wavefunction and $2\ceil{(m+1)/2}$ in the energy.
...
+
  H_\mr{ee}
$,\,\footnote{We are temporarily dropping the subscript $e$ for clarity.}
with
$
  \Y_\mr{x}
\equiv
...
=
  ...
  \ip{\bo{\F}_{m+1}\ \bo{\F}_{m+2}|E_e - H_e|\bo{\F}_{m+1}\ \bo{\F}_{m+2}}^{-1}
  \ip{\bo{\F}_{m+1}\ \bo{\F}_{m+2}|H(C_{m-1} + C_m)|\F}
\end{align}
where we have used Slater's rules to eliminate matrix elements of $H_e$ where the determinants differ by more than two orbitals.
...
\begin{align}
  E_e^{\{1\}}
=
  \ip{\ol{\Y}_\mr{i}^{\{0\}}|\ol{H}_e|\bo{\F}_\mr{e}}
  \ip{\bo{\F}_\mr{e}|E_e - \ol{H}_e|\bo{\F}_\mr{e}}^{-1}
  \ip{\bo{\F}_\mr{e}|\ol{H}_e|\ol{\Y}_\mr{i}^{\{0\}}}
&&
\begin{array}{r@{\ }l}
  \kt{\ol{\Y}_\mr{i}^{\{0\}}}
&=
...
Using
$
  1_\mr{i}(E_e + \ol{H}_0) 1_\mr{e}
=
  0
$
and
$
  1_\mr{e}(E_e + \ol{H}_0)\kt{\F}
=
  0
$
...
This approximation to $E_e^{\{1\}}$ defines the
$
  (m+1)_\La
$
correction
