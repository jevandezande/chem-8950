...
\begin{dfn}\thmtitle{Resolvent line (extended definition)}
...
\vspace{5pt}
\thmproof{
  This follows directly from Wick's theorem and the definition of a resolvent line.
}
\end{cor}
...
\begin{ex}
Equations \ref{eq:energy-substitution-psi-3} and \ref{eq:energy-substitution-psi-4} are trivially consistent with the Bracketing theorem
\begin{align}
  \Y\ord{3}
=&\
  R_0V_\mr{c}R_0V_\mr{c}R_0V_\mr{c}\F
...
-
  R_0\ip{V_\mr{c}R_0V_\mr{c}R_0V_\mr{c}}R_0V_\mr{c}\F
\\
\intertext{since $E_\mr{c}\ord{2}=\ip{V_\mr{c}R_0V_\mr{c}}$ and $E_\mr{c}\ord{3}=\ip{V_\mr{c}R_0V_\mr{c}R_0V_\mr{c}}$.
...
\begin{dfn}
\thmtitle{Separate graph}
The parts of a disconnected graph are termed \textit{separate} if they each have their own sets of resolvent lines.
Otherwise, they form part of the same \textit{combined} graph.
\end{dfn}

\begin{dfn}
\thmtitle{Level}
Each vertical position in a graph containing an interaction line or resolvent line defines a \textit{level}.
In other words, two operators share the same level if their interactions lie on the same horizontal line.
Levels are numbered from top to bottom, so that the \textit{first level} refers to the topmost operator or resolvent.
If two separate parts are drawn so that none of their operators or resolvents occupy the same level, we define the \textit{corresponding combined graph} as the result of extending the resolvent lines across both parts.
We can form different combined graphs corresponding to the same separated graph by changing the relative vertical ordering of its parts.
\end{dfn}

\begin{dfn}
\thmtitle{Insertion graph}
A separate, unlinked part in a graph is termed an \textit{insertion}.
The rest of the graph is termed the \textit{remainder}.
In the context of perturbation theory, inserted graphs come from the expectation values in the bracketing expansion.
The position of each left angle bracket in the bracketing expansion is referred to as the \textit{level of the insertion}.
The \textit{corresponding set} of a graph with an insertion contains corresponding graphs for all relative vertical orderings, keeping the top of the inserted graph at the level of the insertion.
\end{dfn}
...
\thmtitle{Infinite Recursion Formula for $\Y(\la)$}
\thmstatement{
$\ds{
  \Y(\la)
=
...
=&\
  ...
  \la^m
  (-)^k\,\,
  {}^{k+r_{k+1}}\hspace{-1pt}C
  (
    R_0V_\mr{c}:
    R_0E_\mr{c}\ord{r_1},\ld,
    R_0E_\mr{c}\ord{r_k}
  )
  \F
\end{align*}}%
where we have used a multi-sum reduction to group constant powers of $\la$.
Writing the two inner sums as a single sum over $\mc{C}(m)$ and differentiating with respect to $\la$ completes the proof.
\begin{align}
  \Y\ord{m}
=
  \left.
