...
and labelling these creation and annihilation lines with $p_1,\ld,p_m$ and $q_1,\cd,q_m$, respectively.
...
\begin{enumerate}
\item
  Each set of $k$ equivalent internal lines or equivalent subgraphs contributes a factor of $k!$ to the degeneracy.\\
  (If rule~\ref{item:rule-for-coefficient-lines} is used, this excludes equivalent coefficient lines.  Otherwise, replace rule~\ref{item:rule-for-coefficient-lines} with the full antisymmetrizer.)
\item
...
\item\label{item:rule-for-coefficient-lines}
  In a coefficient graph, antisymmetrize over inequivalent coefficient lines using the reduced\footnotemark\ antisymmetrizer
  $\op{P}^{(P_1/\cd /P_h)}_{(Q_1/\cd /Q_k)}$
  where the $P_i$'s and $Q_i$'s label subsets of equivalent coefficient lines.
\end{enumerate}
\end{rmk}
\footnotetext{
  The full antisymmetrizer would be $\op{P}^{(p_1/\cd/p_m)}_{(q_1/\cd/q_m)}$, where
  $\{p_1,\ld,p_m\}=P_1\cup\cd\cup P_h$ and
  $\{q_1,\ld,q_m\}=Q_1\cup\cd\cup Q_k$ are the upper and lower indices of the bare excitation operator.
}
