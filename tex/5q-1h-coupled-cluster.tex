...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE,RO]{Quiz 5, Handout 1: Diagrams}
...
A \textit{wave operator} maps a determinant into a correlated wavefunction, $\Y=\op{\W}\F$.
...
\thmproof{
This follows from a direct Taylor expansion of the exponentials, along with proposition~\ref{prop:nested-commutator}.
\begin{align*}
  e^XYe^{-X}
=
  \sum_{h=0}^\infty
...
=
  ...
  [X,\,\cdot\,](Y)
\end{align*}
In the second step, we have rearranged the sum to run over $n=h+k$ and $k$.
}
\end{thm}
...
First, note that $a^p_p\F_\mu = n_p^\mu \F_\mu$, where $n_p^\mu$ denotes the occupation of $\y_p$ in $\F_\mu$.
By Wick's theorem, $a^p_p = \tl{a}^p_p + n_p^0$, where $n_p^0$ denotes the occupation of $\y_p$ in $\F$.
Therefore, $\tl{a}^p_p\F_\mu = (n_p^\mu - n_p^0)\F_\mu$ and $H_0\F_\mu = \pr{\sum_{p\in\F_\mu}f_p^p - \sum_{p\in\F}f_p^p}\F_\mu$.
}
...
For example, the excitation levels of the singles, doubles, and triples cluster operators are one, two, and three.
The excitation levels of the particle and quasi-particle Hamiltonian diagrams are as follows.
\begin{align*}
\diagram{
  \draw (-0.5,0) node[circlep] (h) {} -- (0,0) node[ddot=white] (h1) {};
  \draw[->-] (h1) to ++(0,+0.5);
...
\end{align*}
A convenient rule for evaluating expectation values is that the total excitation level of a closed graph must be zero.
This can be used to identify which terms in the quasi-particle Hamiltonian are required to complete a contraction.
\end{dfn}
...
For $n=0$ this follows from the definition of the commutator,
$
  [X,Y]
=
  XY
