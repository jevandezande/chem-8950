...
\end{array}
\end{align}
Note that $E_\mr{ref}$ is another expression for the Hartree-Fock energy, $E_\mr{ref}=\ip{\F|\op{H}_e|\F}$, and $f_{pq}$ is the matrix representation of the Fock operator in the spin-orbital basis, $\bm{f}=[f_{pq}]$ where $f_{pq}=\ip{\y_p|\op{f}|\y_q}$.
