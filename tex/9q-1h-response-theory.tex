...
\begin{ex}
Substituting eq~\ref{eq:interaction-time-envelopes} into eq~\ref{eq:wavefunction-dyson-series} and comparing the result to eq~\ref{eq:general-perturbation-expansion} yields the wavefunction responses.
\begin{align}
...
=
  ...
  \Y_0
\end{align}
Substituting this expansion into $\ip{\Y(t)|W|\Y(t)}=\ip{\tl{\Y}(t)|\tl{W}(t)|\tl{\Y}(t)}$ and grouping powers of $\bm{f}$ gives the following
\begin{align}
...
\end{align}
which can be further simplified using the permutational symmetries of the wavefunction responses.
\end{ex}

\begin{prop}
\label{prop:linear-response-commutator-expression}
\thmtitle{Linear response function}
\thmstatement{
$\ds{
  \iip{\tl{W}(t);\tl{V}_\b(t')}
=
...
\begin{samepage}
\begin{cor}
\thmtitle{Linear response function (spectral resolution)}
\thmstatement{
The linear response function can be expressed as
\begin{align*}
  \iip{\tl{W}(t);\tl{V}_\b(t')}
=
-
...
\end{align*}
where $\w_k\equiv E_k-E_0$ is the excitation energy of the $k\eth$ stationary state, $H\Y_k=E_k\Y_k$.
}\vspace{5pt}
\thmproof{
...
\end{align}
where we have used $H\Y_0=E_0\Y_0$.
The proposition follows by inserting resolution of the identity 
}
\end{cor}
\end{samepage}
