...
the points $\bo{x}$ and $\bo{x}'$ are distinct if and only if $r=\|\bo{x}-\bo{x}'\|/2$ is a positive number.
  If so, then $\mb{B}_r(\bo{x})$ and $\mb{B}_r(\bo{x}')$ are disjoint open subsets containing each and they are separated by neighborhoods.
}
\end{prop}
...
\begin{prop}
\thmstatement{
The cotangent $\a_\bo{x}$ in $T_\bo{x}^*\mb{R}^n$ is an exact differential at if and only if its expansion coefficients in the standard basis have the form $A_\bo{x}^i=(\pt_{x_i})_\bo{x}(f)$ for some smooth function $f$ in $C^\infty(\mb{R}^n)$.
}\vspace{5pt}
\thmproof{
  Using equations~\ref{eq:cotangent-tangent-dot-product-expansion} and \ref{eq:tangent-cotangent-expansions}, we can show the cotangent with these expansion coefficients is equal to $(df)_\bo{x}$.
\begin{align}
  \a_\bo{x}(\mc{D}_\bo{x})
=
  \sum_i
